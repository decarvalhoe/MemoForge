...
describe('progression — déverrouillage', () => {
	...
	test('la carte est complète : plus aucune région vide, la chaîne se déroule entière', () => {
		// La « vacuité » (région sans niveau, résolue d'office) était le mécanisme de
		// transition pendant la construction de la carte — toutes les régions ont
		// désormais du contenu. On vérifie la complétude et le déroulé de bout en bout.
		for (const r of REGIONS)
			assert.ok(r.levelIds.length >= 1, `région ${r.id} sans niveau`);
		const all = idsOf(LEVELS.map((l) => l.id));
		for (const r of REGIONS) {
			assert.ok(isRegionUnlocked(r.id, all), `${r.id} devrait être déverrouillée`);
			assert.equal(regionStatus(r.id, all), 'solved');
		}
		assert.equal(currentRegion(all), null, 'tout résolu → plus de région courante');
	});
